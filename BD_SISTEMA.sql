-- INICIO DELETA BASE DE DADOS
USE MASTER
GO
DROP DATABASE BD_SISTEMA
-- FIM DELETA BASE DE DADOS

-- INICIO CRIAÇÃO BANCO DE DADOS
CREATE DATABASE BD_SISTEMA
GO

USE BD_SISTEMA
GO
-- FIM CRIAÇÃO BANCO DE DADOS

-- INICIO CRIAÇÃO TABELAS
CREATE TABLE ESTADO (
...
CREATE TABLE CNH (
	idCNH INT NOT NULL IDENTITY,
	numeroRegistro VARCHAR(11) NOT NULL UNIQUE,
...
CREATE TABLE MOTORISTA (
	...
	FOREIGN KEY (idPessoa) REFERENCES PESSOA (idPessoa),
	FOREIGN KEY (idCNH) REFERENCES CNH (idCNH)
)
GO
...
CREATE TABLE COMBUSTIVEL (
	idCombustivel INT NOT NULL IDENTITY,
	combustivel VARCHAR(25) NOT NULL UNIQUE,
...
CREATE TABLE INFRACAO (
	idInfracao INT NOT NULL IDENTITY,
	infracao VARCHAR(50) NOT NULL UNIQUE,
	classificacao VARCHAR(15) NOT NULL,
	CHECK (classificacao = 'LEVE' OR classificacao = 'MÉDIA' 
			or classificacao = 'GRAVE' or classificacao = 'GRAVÍSSIMA'),
...
CREATE TABLE VEICULO (
	...
	categoriaExigida VARCHAR(2) NOT NULL,
	CHECK (categoriaExigida = 'A' 
		OR categoriaExigida = 'B' 
		OR categoriaExigida = 'C'
		OR categoriaExigida = 'D'
		OR categoriaExigida = 'E'),
...
CREATE TABLE SERVICO (
	...
	servico VARCHAR(25) NOT NULL UNIQUE,
...
CREATE TABLE UNIDADEMEDIDA (
	...
	unidadeMedida CHAR(2) NOT NULL UNIQUE,
...
CREATE TABLE MANUTENCAO (
	idManutencao INT NOT NULL IDENTITY,
...
CREATE TABLE PRODUTO_MANUTENCAO (
	...
	PRIMARY KEY (idManutencao, idProduto),
...
CREATE TABLE SERVICO_MANUTENCAO (
	...
	valor DECIMAL(9, 2) NOT NULL,
	PRIMARY KEY (idManutencao, idServico),
	FOREIGN KEY (idManutencao) REFERENCES MANUTENCAO (idManutencao),
...
AS 
SELECT USUARIO.idUsuario, USUARIO.usuario, USUARIO.senha, CARGO.*, EMPRESA.idEmpresa, EMPRESA.razaoSocial, EMPRESA.nomeFantasia, EMPRESA.cnpj, EMPRESA.ie, EMPRESA.im, EMPRESA.dataAbertura, CIDADE.cidade, ESTADO.*, PESSOA.* FROM USUARIO
...
CREATE VIEW VW_SELECIONA_VEICULO 
AS 
SELECT VEICULO.*, COR.cor, MODELO.modelo, MARCA.*, COMBUSTIVEL.combustivel FROM VEICULO
...
CREATE PROCEDURE SP_OBTEM_PRODUTOS_MANUTENCAO (@IdManutencao INT) 
AS
	SELECT 
		PRODUTO_MANUTENCAO.*, PRODUTO.produto, UNIDADEMEDIDA.* 
	FROM 
		PRODUTO_MANUTENCAO 
	INNER JOIN PRODUTO ON PRODUTO_MANUTENCAO.idProduto = PRODUTO.idProduto
	INNER JOIN UNIDADEMEDIDA ON PRODUTO.idUnidadeMedida = UNIDADEMEDIDA.idUnidadeMedida
	WHERE 
		idManutencao = @IdManutencao
GO

CREATE PROCEDURE SP_OBTEM_SERVICOS_MANUTENCAO (@IdManutencao INT) 
AS
	SELECT 
		SERVICO_MANUTENCAO.*, SERVICO.servico 
	FROM 
		SERVICO_MANUTENCAO 
	INNER JOIN SERVICO ON SERVICO_MANUTENCAO.idServico = SERVICO.idServico
	WHERE 
		idManutencao= @IdManutencao
GO

CREATE PROCEDURE SP_OBTEM_MODELOS (@IdMarca INT) 
AS
	SELECT 
		MODELO.*, MARCA.marca 
	FROM 
...
CREATE PROCEDURE SP_SELECIONA_CNH_IRA_VENCER
AS
	SELECT
		MOTORISTA.*, CNH.numeroRegistro, CNH.dataValidade, CNH.categoria
	FROM motorista
		INNER JOIN CNH ON MOTORISTA.idCNH = CNH.idCNH
		WHERE CNH.dataValidade <= DATEADD(MONTH, 1, SYSDATETIME())
	ORDER BY CNH.dataValidade
GO

CREATE PROCEDURE SP_SELECIONA_MULTA_IRA_VENCER
AS
	SELECT 
		MULTA.idMulta, MULTA.idVeiculo, MULTA.idMotorista, MULTA.valor, MULTA.dataVencimento, VEICULO.placa, MOTORISTA.nome
	FROM 
		MULTA
	INNER JOIN VEICULO ON VEICULO.idVeiculo = MULTA.idVeiculo
	INNER JOIN MOTORISTA ON MOTORISTA.idMotorista = MULTA.idMotorista
		WHERE MULTA.dataVencimento <= DATEADD(MONTH, 1, SYSDATETIME())
	ORDER BY MULTA.dataVencimento
GO
...
INNER JOIN CIDADE ON PESSOA.idCidade = CIDADE.idCidade 
INNER JOIN ESTADO ON CIDADE.idEstado = ESTADO.idEstado
INNER JOIN EMPRESA ON EMPRESA.idPessoa = PESSOA.idPessoa
GO

CREATE VIEW VW_SELECIONA_CARGO AS SELECT * FROM CARGO
GO

CREATE VIEW VW_SELECIONA_MARCA AS SELECT * FROM MARCA
GO

CREATE VIEW VW_SELECIONA_INFRACOES AS SELECT * FROM INFRACAO
GO

CREATE VIEW VW_SELECIONA_MULTAS AS 
	SELECT 
		MULTA.*, ESTADO.idEstado, INFRACAO.infracao, VEICULO.placa, MOTORISTA.nome
	FROM 
		MULTA
...
CREATE VIEW VW_SELECIONA_COMBUSTIVEL AS SELECT * FROM COMBUSTIVEL
GO

CREATE VIEW VW_SELECIONA_PRODUTO AS 
	SELECT PRODUTO.*, UNIDADEMEDIDA.unidadeMedida FROM PRODUTO
	INNER JOIN UNIDADEMEDIDA ON UNIDADEMEDIDA.idUnidadeMedida = PRODUTO.idUnidadeMedida
GO

CREATE VIEW VW_SELECIONA_UNIDADEMEDIDA AS SELECT * FROM UNIDADEMEDIDA
GO

CREATE VIEW VW_SELECIONA_SERVICO AS SELECT * FROM SERVICO
GO

CREATE VIEW VW_SELECIONA_COR AS SELECT * FROM COR
GO

CREATE VIEW VW_SELECIONA_MOTORISTA
AS
SELECT PESSOA.*, CIDADE.cidade, ESTADO.*, MOTORISTA.idMotorista, MOTORISTA.dataCadastro, MOTORISTA.nome, MOTORISTA.sexo, MOTORISTA.cpf, MOTORISTA.rg, MOTORISTA.dataNascimento, MOTORISTA.apelido, CNH.*
FROM PESSOA
INNER JOIN CIDADE ON PESSOA.idCidade = CIDADE.idCidade 
INNER JOIN ESTADO ON CIDADE.idEstado = ESTADO.idEstado
INNER JOIN MOTORISTA ON MOTORISTA.idPessoa = PESSOA.idPessoa
...
CREATE PROCEDURE SP_INSERE_VEICULO (@idModelo INT, @placa CHAR(8), @idCor INT, @anoFabricacao DATE, @anoModelo DATE, @km INT, @renavam CHAR(11), @idCombustivel INT, @categoriaExigida VARCHAR(2))
AS
	INSERT INTO
		VEICULO (idModelo, placa, idCor, anoFabricacao, anoModelo, km, renavam, idCombustivel, categoriaExigida)
	VALUES
		(@idModelo, @placa, @idCor, @anoFabricacao, @anoModelo, @km, @renavam, @idCombustivel, @categoriaExigida)
GO

CREATE PROCEDURE SP_ALTERA_VEICULO (@idVeiculo INT, @idModelo INT, @placa CHAR(8), @idCor INT, @anoFabricacao DATE, @anoModelo DATE, @km INT, @renavam CHAR(11), @idCombustivel INT, @categoriaExigida VARCHAR(2))
AS
	UPDATE
		VEICULO 
	SET
		idModelo = @idModelo, placa = @placa, idCor = @idCor, anoFabricacao = @anoFabricacao, anoModelo = @anoModelo, km = @km, renavam = @renavam, idCombustivel = @idCombustivel, categoriaExigida = @categoriaExigida
	WHERE 
		idVeiculo = @idVeiculo
GO

CREATE PROCEDURE SP_DELETA_VEICULO (@idVeiculo INT)
AS	
	DELETE FROM 
		VEICULO
	WHERE 
...
CREATE PROCEDURE SP_ALTERA_EMPRESA (@idPessoa INT, @idEmpresa INT, @logradouro VARCHAR(70), @numero VARCHAR(8),
...
AS
	...
	UPDATE
		EMPRESA
	SET razaoSocial = @razaoSocial, nomeFantasia = @nomeFantasia, cnpj = @cnpj, ie = @ie, im = @im, 
...
AS
	UPDATE 
		CARGO
	SET 
		cargo = @Cargo, permissoes = @Permissoes 
	WHERE 
		idCargo = @idCargo
GO
...
CREATE PROCEDURE SP_INSERE_MARCA (@Marca VARCHAR(25))
AS
	INSERT INTO 
		MARCA (MARCA) 
	VALUES 
...
CREATE PROCEDURE SP_ALTERA_MARCA (@idMarca INT, @Marca VARCHAR(25))
AS
	UPDATE 
		MARCA
	SET 
...
CREATE PROCEDURE SP_DELETA_MARCA (@idMarca INT)
AS	
	DELETE FROM 
		MARCA
	WHERE 
...
CREATE PROCEDURE SP_INSERE_INFRACAO (@Infracao VARCHAR(50), @Classificacao VARCHAR(15))
AS
	INSERT INTO 
		INFRACAO (infracao, classificacao) 
	VALUES 
		(@Infracao, @Classificacao)
GO

CREATE PROCEDURE SP_ALTERA_INFRACAO (@IdInfracao INT, @Infracao VARCHAR(50), @Classificacao VARCHAR(15))
AS
	UPDATE 
		INFRACAO
	SET 
		infracao = @Infracao, classificacao = @Classificacao
	WHERE 
		idInfracao = @IdInfracao
GO

CREATE PROCEDURE SP_DELETA_INFRACAO (@IdInfracao INT)
AS	
	DELETE FROM 
		INFRACAO
	WHERE 
...
CREATE PROCEDURE SP_INSERE_MULTA (@IdVeiculo INT, @IdMotorista INT, @DataInfracao DATETIME, @IdInfracao INT, @Valor DECIMAL(9, 2), @DataVencimento DATE, @DataPagamento DATE, @idCidade INT)
AS
	INSERT INTO 
		MULTA (idVeiculo, idMotorista, dataInfracao, idInfracao, valor, dataVencimento, dataPagamento, idCidade) 
	VALUES 
		(@IdVeiculo, @IdMotorista, @DataInfracao, @IdInfracao, @Valor, @DataVencimento, @DataPagamento, @idCidade)
GO

CREATE PROCEDURE SP_ALTERA_MULTA (@IdMulta INT, @IdVeiculo INT, @IdMotorista INT, @DataInfracao DATETIME, @IdInfracao INT, @Valor DECIMAL(9, 2), @DataVencimento DATE, @DataPagamento DATE, @idCidade INT)
AS
	UPDATE 
		MULTA
	SET 
		idVeiculo = @IdVeiculo, idMotorista = @IdVeiculo, dataInfracao = @DataInfracao, idInfracao = @IdInfracao, valor = @Valor, dataVencimento = @DataVencimento, dataPagamento = @DataPagamento, idCidade = @idCidade
	WHERE 
		idMulta = @IdMulta
GO

CREATE PROCEDURE SP_DELETA_MULTA (@IdMulta INT)
AS	
	DELETE FROM 
		MULTA 
	WHERE 
...
AS
	INSERT INTO 
		PRODUTO_MANUTENCAO(idManutencao, idProduto, quantidade, valorUnitario) 
	VALUES 
		(@IdManutencao, @IdProduto, @Quantidade, @ValorUnitario)
GO

CREATE PROCEDURE SP_ALTERA_PRODUTO_MANUTENCAO (@IdManutencao INT, @IdProduto INT, @Quantidade DECIMAL(9, 2), @ValorUnitario DECIMAL(9, 2))
AS
	UPDATE 
		PRODUTO_MANUTENCAO
	SET 
	...
	WHERE 
		idManutencao = @IdManutencao AND idProduto = @IdProduto
GO

CREATE PROCEDURE SP_DELETA_PRODUTO_MANUTENCAO (@IdManutencao INT, @IdProduto INT)
AS	
	DELETE FROM 
		PRODUTO_MANUTENCAO
	WHERE 
...
CREATE PROCEDURE SP_INSERE_SERVICO_MANUTENCAO (@IdManutencao INT, @IdServico INT, @Valor DECIMAL(9, 2))
AS
	INSERT INTO 
		SERVICO_MANUTENCAO(idManutencao, idServico, valor) 
	VALUES 
		(@IdManutencao, @IdServico, @Valor)
GO

CREATE PROCEDURE SP_ALTERA_SERVICO_MANUTENCAO (@IdManutencao INT, @IdServico INT, @Valor DECIMAL(9, 2))
AS
	UPDATE 
		SERVICO_MANUTENCAO 
	SET 
		valor = @Valor
	WHERE 
		idManutencao = @IdManutencao AND idServico = @IdServico
GO

CREATE PROCEDURE SP_DELETA_SERVICO_MANUTENCAO (@IdManutencao INT, @IdServico INT)
AS	
	DELETE FROM 
		SERVICO_MANUTENCAO
	WHERE 
...
CREATE PROCEDURE SP_INSERE_COMBUSTIVEL (@Combustivel VARCHAR(25))
AS
	INSERT INTO 
...
CREATE PROCEDURE SP_ALTERA_COMBUSTIVEL (@IdCombustivel INT, @Combustivel VARCHAR(25))
AS
	UPDATE 
		COMBUSTIVEL
	SET 
...
CREATE PROCEDURE SP_DELETA_COMBUSTIVEL (@IdCombustivel INT)
AS	
	DELETE FROM 
		COMBUSTIVEL
	WHERE 
...
CREATE PROCEDURE SP_INSERE_SERVICO (@Servico VARCHAR(25))
AS
	INSERT INTO 
		SERVICO (servico) 
	VALUES 
...
CREATE PROCEDURE SP_ALTERA_SERVICO (@idServico INT, @Servico VARCHAR(25))
AS
	UPDATE 
		SERVICO
	SET 
...
CREATE PROCEDURE SP_DELETA_SERVICO (@idServico INT)
AS	
	DELETE FROM 
		SERVICO
	WHERE 
...
CREATE PROCEDURE SP_INSERE_UNIDADEMEDIDA (@UnidadeMedida CHAR(2))
AS
	INSERT INTO 
...
CREATE PROCEDURE SP_ALTERA_UNIDADEMEDIDA (@idUnidadeMedida INT, @UnidadeMedida CHAR(2))
AS
	UPDATE 
		UNIDADEMEDIDA
	SET 
...
CREATE PROCEDURE SP_DELETA_UNIDADEMEDIDA (@idUnidadeMedida INT)
AS	
	DELETE FROM 
		UNIDADEMEDIDA
	WHERE 
...
CREATE PROCEDURE SP_INSERE_MODELO (@idMarca INT, @Modelo VARCHAR(25))
AS
	INSERT INTO 
		MODELO (idMarca, modelo) 
	VALUES 
		(@idMarca, @Modelo)
GO

CREATE PROCEDURE SP_ALTERA_MODELO (@idModelo INT, @idMarca INT, @Modelo VARCHAR(25))
AS
	UPDATE 
		MODELO
	SET 
...
CREATE PROCEDURE SP_DELETA_MODELO (@idModelo INT)
AS	
	DELETE FROM 
		MODELO
	WHERE 
...
CREATE PROCEDURE SP_INSERE_PRODUTO (@IdUnidadeMedida INT, @Produto VARCHAR(25))
AS
	INSERT INTO 
		PRODUTO(idUnidadeMedida, produto) 
	VALUES 
		(@IdUnidadeMedida, @Produto)
GO

CREATE PROCEDURE SP_ALTERA_PRODUTO (@idProduto INT, @idUnidadeMedida INT, @Produto VARCHAR(25))
AS
	UPDATE 
		PRODUTO
	SET 
		idUnidadeMedida = @idUnidadeMedida, produto = @Produto
	WHERE 
		idProduto = @idProduto
GO
...
CREATE PROCEDURE SP_INSERE_COR (@Cor VARCHAR(25))
AS
	INSERT INTO 
		COR (cor) 
	VALUES 
...
CREATE PROCEDURE SP_ALTERA_COR (@idCor INT, @Cor VARCHAR(25))
AS
	UPDATE 
		COR
	SET 
...
CREATE PROCEDURE SP_DELETA_COR (@idCor INT)
AS	
	DELETE FROM 
		COR
	WHERE 
...
CREATE PROCEDURE SP_INSERE_MOTORISTA (@logradouro VARCHAR(70), @numero VARCHAR(8), @complemento VARCHAR(30), @bairro VARCHAR(30), @idCidade INT,
									  @cep CHAR(9), @telefoneFixo CHAR(14), @telefoneCelular CHAR(15), @dataCadastro DATETIME,
									  @nome VARCHAR(50), @apelido VARCHAR(50), @sexo CHAR(1), @cpf CHAR(14), @rg VARCHAR(20), @dataNascimento DATE, 
									  @numeroRegistro VARCHAR(11), @dataValidade DATE, @categoria VARCHAR(2))
AS
BEGIN
	INSERT INTO PESSOA (logradouro, numero, complemento, bairro, idCidade, cep, telefoneFixo, telefoneCelular)
				 VALUES (@logradouro, @numero, @complemento, @bairro, @idCidade, @cep, @telefoneFixo, @telefoneCelular)
	
	DECLARE @idPessoa INT = @@IDENTITY
	INSERT INTO CNH (numeroRegistro, dataValidade, categoria)
				VALUES (@numeroRegistro, @dataValidade, @categoria)

	DECLARE @idCNH INT = @@IDENTITY

	INSERT INTO MOTORISTA(nome, apelido, sexo, cpf, rg, dataNascimento, dataCadastro, idPessoa, idCNH)
				   VALUES (@nome, @apelido, @sexo, @cpf, @rg, @dataNascimento, @dataCadastro, @idPessoa, @idCNH)
END
GO

CREATE PROCEDURE SP_ALTERA_MOTORISTA (@idPessoa INT, @idMotorista INT, @idCNH INT, @logradouro VARCHAR(70), @numero VARCHAR(8), @complemento VARCHAR(30), @bairro VARCHAR(30), @idCidade INT,
									  @cep CHAR(9), @telefoneFixo CHAR(14), @telefoneCelular CHAR(15), @dataCadastro DATETIME,
									  @nome VARCHAR(50), @apelido VARCHAR(50), @sexo CHAR(1), @cpf CHAR(14), @rg VARCHAR(20), @dataNascimento DATE, 
									  @numeroRegistro VARCHAR(11), @dataValidade DATE, @categoria VARCHAR(2))
AS
BEGIN
	UPDATE PESSOA SET logradouro = @logradouro, numero = @numero, complemento = @complemento, bairro = @bairro, 
		idCidade = @idCidade, cep = @cep, telefoneFixo = @telefoneFixo, telefoneCelular = @telefoneCelular
...
CREATE PROCEDURE SP_DELETA_MOTORISTA (@idMotorista INT)
AS	
BEGIN
	DECLARE @idPessoa INT, @idCNH INT

	SELECT @idPessoa = idPessoa, @idCNH = idCNH FROM MOTORISTA WHERE idMotorista = @idMotorista
...
-- INICIO POPULAÇÃO DA BASE DE DADOS
...
INSERT INTO USUARIO
	(idUsuario, usuario, senha, idCargo, idEmpresa) 
		VALUES (1, 'GESTOR', '1234', 2 /*ID Cargo GESTOR DE FROTA*/, 1 /*ID Relacionamento Empresa*/)
GO

SET IDENTITY_INSERT USUARIO OFF
GO
-- FIM POPULAÇÃO DA BASE DE DADOS